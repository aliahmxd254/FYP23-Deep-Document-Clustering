...
class SDCN(nn.Module):
    ...
    def __init__(self, n_enc_1, n_enc_2, n_enc_3, n_dec_1, n_dec_2, n_dec_3, 
                n_input, n_z, n_clusters, d_model, max_sequence_length,
                ffn_hidden, num_heads, drop_prob, num_layers=3, v=1):
        ...
        self.z_layer = Linear(d_model, n_z)
        ...
        self.gnn_1 = GNNLayer(n_input, d_model)
        self.gnn_2 = GNNLayer(n_input, d_model)
        self.gnn_3 = GNNLayer(n_input, d_model)
        self.gnn_4 = GNNLayer(n_input, n_z)
    ...
    def forward(self, x, adj):
        ...
        # GCN Module
        h = self.gnn_1(x_temp, adj)  # h1
        # tra2 = tra2 + 0.001 *F.relu(self.ae.enc_2(h+tra1))  # ae update via (h1 + tra1)

        #print(f"t: {attention_list[0].size()}")

        h = self.gnn_2(0.5*h+0.5*attention_list[0], adj)
        # tra3 = tra3 + 0.001 *F.relu(self.ae.enc_3(h+tra2))  # ae update via (h2 + tra2)

        h = self.gnn_3(0.5*h+0.5*attention_list[1], adj)
